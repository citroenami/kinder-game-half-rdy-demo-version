  ...
  if(currentScene.sceneNum === 1){
    ...
    if(woodQuestDone === true){
     ...
     if(leaveAlready < 1){
      ...
      if(prioritiTimeoutId){
        clearTimeout(prioritiTimeoutId);
        clearTimeout(negateTimeout);
        notRightComboMsg = false;
        clearTimeout(notRightComboMsgTimeoutId);

      }
      clueDscriptionElement.
      innerHTML = clueText;
      prioritiTimeoutId = setTimeout(()=>{
        clueDscriptionElement.
      innerHTML = '';
      },3000);
    }
